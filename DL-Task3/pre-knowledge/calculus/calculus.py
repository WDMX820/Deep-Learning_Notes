...
Original file is located at
    https://colab.research.google.com/drive/10f0DsZc4VvnSUHBoY1qXnz9pkK9MsLNm
...
使用链式法则和矩阵的基本导数规则,我们可以得出:

$\nabla_{\mathbf{X}} \|\mathbf{X} \|_F^2 = 2\mathbf{X}$

理由:
1. Frobenius 范数的平方的导数: 对于单个元素的平方 $X_{ij}^2$ ，它的导数是 $2X_{ij}$。
2. 当我们对 ${}\|\mathbf{X} \|_F^2$ 进行求导时，我们实际上是在对所有 $X_{ij}^2$ 的导数进行求和，结果就是 $2X_{ij}$。
3. 线性性: 矩阵的导数与矩阵相乘的性质使得这个结果保持形状一致。
"""

# 练习1: 绘制函数y=f(x)=x^3-1/x和其在x=1处切线的图像
...
# 切线方程
def tangent_line(x):
    return 4 * (x - 1)  # 切线 y = 4(x - 1)

# 定义 x 的取值范围
x_values = np.linspace(0.1, 2, 100)  # 避免 x = 0 的点
f_values = f(x_values)
...
plt.plot(x_values, f_values, label='f(x) = x^3 - 1/x', color='blue')
plt.plot(x_values, tangent_values, label='Tangent at x=1', linestyle='--', color='orange')
plt.scatter(1, 0, color='red')  # 切点
plt.text(1, 0, " (1, 0)", verticalalignment='bottom', horizontalalignment='right')
